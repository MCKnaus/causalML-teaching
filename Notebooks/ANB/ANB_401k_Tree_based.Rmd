...
Random Forest take the average over many trees that are build on random subsamples using only random subsets of covaraites to split.

There are many different implementations of Random Forests. Here we use the `regression_forest` function of the `grf` package because this is also crucial in the causal ML part.

The random forest predictions are much smoother and overcomes the discrete nature of a single tree:
...
Like in the Application Notebook for Lasso, we check how different methods perform out-of-sample by splitting the dataset in 100 random training and test sets to check the out-of-sample $R^2$.
...
A look at the mean $R^2$ reveals that - as expeced - the single tree performs much worse than the two random forests that are very similar:
...
However, Lasso-based methods achieved on average up to 29% $R^2$ compared to 26% of the tree-based methods. Thus, it seems that linear model are slightly better suited to predict wealth in this application.
