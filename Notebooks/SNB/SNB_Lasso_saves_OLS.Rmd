...
We use the same DGP as in the notebook *Supervised ML: Overfitting of OLS and value of training vs. test sample*:

- $p\geq10$ independent and standard normal covariates: $X \sim N(0,I_p)$, where $I_p$ is the $p$-dimensional identity matrix

- The outcome model is $Y = \underbrace{\beta_0 + \beta_1 X_1 + ... + \beta_{10} X_{10}}_{\text{conditional expectation function }f(X)} + e$, where $X_j$ is the $j$-th column of $X$ and $e \sim N(0,1)$
...
# Combine costant and randomly drawn covariates
...
In the beginning the coefficients of the really relevant variables are build up and at some point the excessive overfitting starts. Cross-validation helps to figure out the sweet spot that ensures a good out-of-sample performance. The `cv.glmnet` command uses by default 10-fold cross-validation leading to the following result:
...
Cross-validation for Post-Lasso is implemented in the `causalDML` package:
...
- The Post-Lasso cross-validation curve is more bumpy and has flat regions compared to the Lasso curve. While Lasso builds up coefficients gradually, Post-Lasso gives the full OLS coefficient as soon as the variable is selected. This explains the flat regions of the Post-Lasso curve where no additional variable is selected and thus MSE stays constant.

- This also explains why Post-Lasso produces sparser models compared to Lasso.

<br>

## Fast implementation with `hdm` 

While the cross-validated Post-Lasso is instructive, it is not fast and the `rlasso` function of the `hdm` package implements a faster way to choose the penalty parameter:
...
We replicate the analysis of Notebook *Supervised ML: Overfitting of OLS and value of training vs. test sample* and gradually add covariates to check how out-of-sample performance develops.
...
for (i in 2:p) {
  ...
  # plasso
  temp_plasso = plasso(x_tr[,2:(i+1)],y_tr)
  temp_yhat_tr = predict(temp_plasso,xnew=x_tr[,2:(i+1)])$plasso
  temp_yhat_te = predict(temp_plasso,xnew=x_te[,2:(i+1)])$plasso
  # Calculate the observable MSEs in training and test sample
...
- Investigate the 1SE rule for cross-validation
