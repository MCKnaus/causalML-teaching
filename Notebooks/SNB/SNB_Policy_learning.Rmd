...
We revisit the DGP from Notebooks *SNM_AIPW_DAML* and *SNB_GATE* with zero ATE but highly nonlinear effect heterogeneity:
...
- The potential outcome model of the treated is $\underbrace{sin(X_1)}_{m_0(X)} + \varepsilon$, with $\varepsilon \sim N(0,1)$

- The treatment effect function is $\tau(X) = sin(X_1) - cos(X_1+1/2\pi)$

This means that some simulated individuals benefit from the treatment (assuming that higher outcome is better) and some do not. The optimal policy is given by $\pi^*(x) = 1[X_1 > 0]$ and should be recovered by the methods we discussed in the lecture.
...
- Run a simulation that compares the regret over multiple draws
