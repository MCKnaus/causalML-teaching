...
We store the fitted values as estimates of the CATEs to compare them later with the truth and other methods.
...
All estimators come quite close, but especially the DR-learner shows a lower correlation with the true CATEs.
...
data.frame(MSE = colMeans( (results16k[,-1]-c(tau))^2 ) ,
  ...
  ggplot(aes(x=Method,y=MSE)) + geom_point(size = 2) + 
  ggtitle(paste(toString(n),"observations")) + geom_hline(yintercept = 0)
# save.image("D:/Switch/Teaching/GESIS Spring Seminar/Day 3/Meta_learner.RData")
```

With increasing sample size, the DR-learner with ensemble methods catches up with the methods that are tailored for the specific DGP. *rlasso* and the purely Random Forest based DR-learner continuously improve but are not competitive.
