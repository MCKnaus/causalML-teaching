...
Recall that in matrix notation the parameter estimates are $\beta = (X'X)^{-1}X'Y$, where $X$ constains a constant. Now the fitted value at the test point is $\hat{y}_0 = [1~x_0] \beta = \underbrace{[1~x_0] (X'X)^{-1}X'}_{w_{ols}} Y$, where $w_{ols}$ contains the weights that each outcome receives when forming the fitted value.
...
A global method like OLS uses all available data points to form the prediction and outcomes that are far away of the point of interest receive negative weight.
