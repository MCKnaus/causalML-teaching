...
*Acknowledgements: I thank Henri Pfleiderer for his help in preparing this notebook.*
...
Let's imagine the following DGP: 

$$\begin{align} Y_i= X_i + U_i \ \ \ with \ \ \ U_i \sim \mathcal{N}(0,0.5) \\
X_i = \mathbf{1}(V_i\geq 0) \\
W_i = \mathbf{1}(Z_i\geq 0) \\
...
Let's plot the distributions of $Y$ for different $W=0$ and $W=1$:
...
Let's look at the importance of the above concept in a causal inference context. $Y$ denotes the outcome of interest, $W$ the treatment and $X$ some additional (confounding) variable. $Y(1)$ and $Y(0)$ are the potential outcomes in the treated and untreated case respectively, so $Y = W \cdot Y(1) + (1-W) \cdot Y(0)$.
...
$Y(0)$ and $Y(1)$ are the potential outcomes for the untreated and the treated case. $Y(0) = X + U_0$ with $U_0 \sim \mathcal{N}(0,0.5)$.
...
$$\begin{align}   Y_{i} =
    \begin{cases}
      X_{i} + U_{0i} & \text{if $W_{i} = 0$}\\
      0.5 + X_{i} + U_{1i} & \text{if $W_{i} = 1$}
    \end{cases}  
\end{align}$$
...
First, consider the case with $\rho = 0$ to illustrate independence in the sense that $Y(w) \perp\!\!\!\!\perp W$.
...
$$ Y_i = W_i\cdot (0.5 + X_i + U_{1i}) + (1-W_i) \cdot (X_{i} + U_{0i}) = 0.5W_i + X_i +W_iU_{1i} + (1-W_i)U_{0i} =  0.5W_i + X_i + U_i$$
