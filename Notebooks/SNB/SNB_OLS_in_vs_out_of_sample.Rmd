---
title: "Supervised ML: Overfitting of OLS and value of training vs. test sample"
...
Consider the following simple data generating process (DGP):
...
# Gradually adding covariates and the failure of in-sample measures
...
We calculate and store four quantities: the training and test sample MSE that would be observable in a real application $(Y-\hat{m}(X))^2$ and the respective oracle MSEs $(Y-m(Y))^2$, where we exploit that we defined and thus know the true CEF.
...
The observable and the oracle MSE are nearly identical. This illustrates why using the observable MSE as proxy for predictive performance is so powerful.
